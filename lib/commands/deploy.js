...
const buildZip = async (config, index) => {
  await new Promise((resolve, reject) => {
    ...
    const output = fs
      .createWriteStream(`${process.cwd()}/${config.distPath}/youxin-electron-vue Setup 1.0.0.exe.zip`)
      .on('close', (e) => {
        if (e) {
          error(`打包zip出错: ${e}`)
          reject(e)
    ...
    archive.pipe(output)
    archive.file(config.distPath + '/youxin-electron-vue Setup 1.0.0.exe', false)
    archive.finalize()
  })
}
    ...
    if (checkDeployConfigExists()) {
      ...
      if (env) {
        ...
        const answers = await confirmDeploy(
          `${underline(projectName)} 项目是否部署到1 ${underline(
            envConfig.name
          )}?`
        )
